...
def combine_audio_video(audiofile,videofile,output_file_name_with_extension):
    ...
    if theofficialqual == "4k" or theofficialqual == "8k":
        video.write_videofile(str(download_path+"/"+output_file_name_with_extension), fps=60, threads=16 if os.name == "posix" else 32, codec = "libfdk_aac")
    else: video.write_videofile(str(download_path+"/"+output_file_name_with_extension), fps=30, threads=16 if os.name == "posix" else 32, codec = "libfdk_aac")
