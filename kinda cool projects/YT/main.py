...
def combine_audio_video(audiofile, videofile, output_file_name_with_extension):
    ...
    if theofficialqual == "4k" or theofficialqual == "8k":
        video.write_videofile(str(SAVE_PATH+"/"+output_file_name_with_extension), fps=60, threads=16 if os.name == "posix" else 32, codec = "libfdk_aac")
    else: video.write_videofile(str(SAVE_PATH+"/"+output_file_name_with_extension), fps=30, threads=16 if os.name == "posix" else 32, codec = "libfdk_aac")
...
if os.name == "nt":
    while True:
        os.system('cls')
        l8650388940 = getpass.getpass("username? --> ")
        try:
            with open("C:/Users/aarus/{}.txt".format(l8650388940),"r+") as f5886120618:
                g8782959354 = f5886120618.readlines()[0][::-1]
        except FileNotFoundError:
            print("Incorrect username! ")
            continue
        g9742283799 = getpass.getpass("g974? --> ")
        if g9742283799 == g8782959354:
            break
        else: 
...
while True:
    ...
    print("==========================================================================================")
    textinput = input("Give the link of the video needed to be installed: -->  ") # link of the video
    
    # If we are reading the txt file
    if textinput == "the.txt":
        print("==========================================================================================")
        print("Reading file!" + "\n\n")

        # Read the text file
        with open("/home/aarush/YT/the.txt", "r") as f:
            mylist = [line.rstrip('\n') for line in f]
            mylist = mylist[1:]
        
        # and read line by line
        for line in mylist:
            link, quality = line.split("|")
    ...
    elif textinput[0:3] == "www" or textinput[0:3] == "htt":
